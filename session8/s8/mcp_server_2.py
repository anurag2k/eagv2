...
def search_documents(query: str) -> list[str]:
    ...
    try:
        ...
        for idx in I[0]:
            ...
            # Convert the dictionary into a JSON string and append that
            results.append(json.dumps(result_dict))
        return results
    except Exception as e:
        # Also good to return errors in a JSON format
        return [json.dumps({"error": f"Failed to search: {str(e)}"})]
    ...
    for file in DOC_PATH.glob("*.*"):
        ...
        try:
            ...
            for i, chunk in enumerate(tqdm(chunks, desc=f"Embedding {file.name}")):
                ...
                print(f"Chunk Text: [{chunk.strip()}]") # .strip() helps see if it's just whitespace
                # ---------------------------------------
            ...
            try:
                ...
                # Stop the loop so you can inspect the last printed chunk
                break
